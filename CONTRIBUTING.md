# Contributing to Panoramerge

Thanks for helping improve Panoramerge. Bug reports, focused fixes, documentation improvements, and well-scoped features are welcome.
...
- Search [existing issues](https://github.com/focale-editor/panoramerge/issues) before opening a new one.
...
Panoramerge follows the SDK constraint in [`pubspec.yaml`](pubspec.yaml). Install a compatible stable Dart SDK, fork the repository, and run:

```sh
git clone https://github.com/YOUR_ACCOUNT/panoramerge.git
cd panoramerge
dart pub get
```
...
Image-processing changes should include deterministic tests for registration, seams, blending, or resource limits as applicable.
